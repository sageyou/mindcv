...
def create_optimizer(
    ...
    **kwargs,
    ...
    Args:
        ...
        weight_decay_filter: filters to filter params from weight_decay.
    ...
    if weight_decay_filter == "auto":
        _logger.warning(
            ...
            "NOTE: mindspore.nn.Optimizer will filter BatchNorm parmas from weight decay. "
        )
    elif weight_decay_filter == "disable" or "bn_and_bias":
        params = init_group_params(params, weight_decay, weight_decay_filter)
        weight_decay = 0.0
    else:
        raise ValueError(
            f"weight decay filter only support ['disable', 'auto', 'bn_and_bias'], but got{weight_decay_filter}."
        )
