    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        if self.image_size is not None:
            if self.strict_img_size:
                ...
                assert W == self.image_size[1], f"Input width ({W}) doesn't match model ({self.image_size[1]})."
            elif not self.dynamic_img_pad:
                assert H % self.patch_size[0] == 0, f"Input height ({H}) should be divisible by patch size " \
                                                    f"({self.patch_size[0]})."

                assert W % self.patch_size[1] == 0, f"Input width ({W}) should be divisible by patch size " \
                                                    f"({self.patch_size[1]})."
