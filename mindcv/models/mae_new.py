...
from .vit_new import Block, VisionTransformer
from .layers.mlp import Mlp
from .layers.patch_embed import PatchEmbed

__all__ = [
    "mae_b_16_224_pretrain_new",
    "mae_l_16_224_pretrain",
    "mae_h_16_224_pretrain",
    "mae_b_16_224_finetune_new",
...
class MAEForPretrain(nn.Cell):
    ...
    def _init_weights(self):
        for name, cell in self.cells_and_names():
            ...
            # todo: 这里再nn.Dense的时候不能初始化嘛 还要单独的写一下？
...
@register_model
def mae_b_16_224_pretrain_new(pretrained=False, **kwargs):
...
@register_model
def mae_b_16_224_finetune_new(pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs):
...
def mae_l_16_224_finetune(pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs):
    ...
    model = VisionTransformer(
        image_size=224, patch_size=16, in_chans=in_channels, embed_dim=1024, depth=24, num_heads=16,
        global_pool='avg', norm_layer=partial(nn.LayerNorm, epsilon=1e-6),
        num_classes=num_classes, **kwargs
    )
    if pretrained:
...
def mae_h_14_224_finetune(pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs):
    ...
    model = VisionTransformer(
        image_size=224, patch_size=14, in_chans=in_channels, embed_dim=1280, depth=32, num_heads=16,
        global_pool='avg', norm_layer=partial(nn.LayerNorm, epsilon=1e-6),
        num_classes=num_classes, **kwargs
    )
    if pretrained:
